...
function my_script_init() { // my_script_init 関数の定義
	...
	wp_enqueue_style('swiper-css', '//cdn.jsdelivr.net/npm/swiper@8.1.4/swiper-bundle.min.css', array(), '8.1.4', 'all');


	// 外部のGoogle Fontsからフォントを読み込む
	wp_enqueue_style( 'google-fonts', '//fonts.googleapis.com/css2?family=Lato:wght@700&family=Montserrat:wght@400;500;700&family=Noto+Sans+JP:wght@400;500;700&display=swap', array(), null );
	...
	wp_enqueue_script('swiper-js', '//cdn.jsdelivr.net/npm/swiper@8.1.4/swiper-bundle.min.js', array('jquery'), '8.1.4', true);
...
///// カテゴリー編集画面に背景色フィールドを追加 /////
function add_category_background_field($tag) {
	$background_color = get_term_meta($tag->term_id, 'background_color', true);
	?>
	<tr class="form-field">
			<th scope="row" valign="top"><label for="background_color">背景色</label></th>
			<td>
					<input type="color" name="background_color" id="background_color" value="<?php echo esc_attr($background_color) ? esc_attr($background_color) : '#ffffff'; ?>">
					<p class="description">このカテゴリーの背景色を選択してください。</p>
			</td>
	</tr>
	<?php
}
add_action('category_edit_form_fields', 'add_category_background_field');



///// 背景色を保存 /////
function save_category_background_field($term_id) {
	if (isset($_POST['background_color'])) {
			update_term_meta($term_id, 'background_color', sanitize_hex_color($_POST['background_color']));
	}
}
add_action('edited_category', 'save_category_background_field');
...
///// メッセージカテゴリー編集画面に背景色フィールドを追加 /////
function add_message_category_background_field($tag) {
	$background_color = get_term_meta($tag->term_id, 'background_color', true);
	
	// デバッグ用ログ
	error_log("Retrieved Background Color: " . print_r($background_color, true));
	
	if (empty($background_color)) {
			$background_color = '#ffffff'; // デフォルトの背景色
	}
	?>
	<tr class="form-field">
			<th scope="row" valign="top"><label for="background_color--message">背景色</label></th>
			<td>
					<input type="color" name="background_color" id="background_color--message" value="<?php echo esc_attr($background_color); ?>">
					<p class="description">このメッセージカテゴリーの背景色を選択してください。</p>
			</td>
	</tr>
	<?php
}
add_action('message_category_edit_form_fields', 'add_message_category_background_field');

///// メッセージカテゴリーの背景色を保存 /////
function save_taxonomy_background_field($term_id) {
	if (isset($_POST['background_color'])) {
			$sanitized_color = sanitize_hex_color($_POST['background_color']);
			if (!empty($sanitized_color)) {
					update_term_meta($term_id, 'background_color', $sanitized_color);
			}
			
			// デバッグ用ログ
			error_log("Term ID: $term_id, Saved Background Color: $sanitized_color");
	}
}
add_action('edited_message_category', 'save_taxonomy_background_field');
